...
void MainWindow::StartRace(void){
    ...
    {        
        futrac1=QtConcurrent::run([&]{concurRace1->DoWork(&number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value());});
        futrac1.waitForFinished();

        futrac1=QtConcurrent::run([&]{concurRace2->DoWork(&number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value());});
        futrac2.waitForFinished();
    }
    else
    {
